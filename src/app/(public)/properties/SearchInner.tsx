...
interface Props {
  initialListings: UiListing[];
  initialLocations: string[];
  serverCampusId: string;
}

export default function SearchInner({ initialListings, initialLocations, serverCampusId }: Props) {
  ...
  const [all, setAll] = useState<UiListing[]>(initialListings);
  ...
  useEffect(() => {
    if (initialRender.current) {
      ...
      if (campus.id === serverCampusId) return;
    }
    ...
    return () => { active = false; };
  }, [campus.id]); // eslint-disable-line react-hooks/exhaustive-deps
